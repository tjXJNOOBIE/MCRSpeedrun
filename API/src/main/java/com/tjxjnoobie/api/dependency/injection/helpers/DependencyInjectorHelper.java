...
public class DependencyInjectorHelper extends AbstractContext<IContext<?>> implements IDependencyInjectorHelper {
    ...
    private Set<String> collectPackages(Class<?> targetClass) {
        // Use the allowed package prefixes defined by the injection configuration.
    ...
    public void walkDirectoryForInjectables(File dir, String packageName, Set<Class<?>> results) {
        ...
        for (File file : files) {
            if (file.isDirectory()) {
                // Step 2: Explore subdirectories to ensure nested packages are processed.
                walkDirectoryForInjectables(file, packageName + "." + file.getName(), results);
            } else if (file.getName().endsWith(".class")) {
                String className = packageName + '.' + file.getName().replace(".class", "");
                try {
                    // Step 3: Load class metadata and add eligible entries to the result set.
    ...
    public Set<Class<?>> findImplementations(Class<?> interfaceType, String basePackage) {
        ...
        // Step 1: Translate the package name into a classpath resource path.
        String path = basePackage.replace('.', '/');

        try {
            // Step 2: Iterate over all resources representing the package.
            Enumeration<URL> resources = Thread.currentThread().getContextClassLoader().getResources(path);
            while (resources.hasMoreElements()) {
                URL resource = resources.nextElement();
                File dir = new File(resource.getFile());
            ...
            // Step 3: Report failures when traversing package resources.
    ...
    /**
     * Recursively walks a directory collecting assignable, concrete classes.
     *
     * @param interfaceType the parent type that discovered classes must extend or implement
     * @param dir           the directory to traverse
     * @param packageName   the package name associated with the directory
     * @param results       the collection to populate with discovered classes
     */
    @Override
