...
public class Ban implements SimpleCommand {
    ...
    public Ban(GlobalContext globalContext, ProxyServer proxyServer) {
    ...
    public void execute(Invocation invocation)  {
        ...
        if (length < 1 || length > 3) {
            source.sendMessage(proxyUtils.withStaffPrefix("&cUsage: /ban <player> [duration] [reason]"));
            return;
        }
     ...
     */
    private void handleBan(CommandSource source, Optional<Player> targetPlayer, String targetName, String reason, String durationStr) {
     ...
     */
    private Duration determineBanDuration(String durationStr, CommandSource source, ProxyUtils proxyUtils) {
